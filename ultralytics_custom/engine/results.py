...
from ultralytics_custom.data.augment import LetterBox
from ultralytics_custom.utils import LOGGER, SimpleClass, ops
from ultralytics_custom.utils.checks import check_requirements
from ultralytics_custom.utils.plotting import Annotator, colors, save_one_box
from ultralytics_custom.utils.torch_utils import smart_inference_mode
...
class BaseTensor(SimpleClass):
    ...
    def cuda(self):
        ...
        Examples:
            >>> import torch
            >>> from ultralytics_custom.engine.results import BaseTensor
            >>> data = torch.tensor([[1, 2, 3], [4, 5, 6]])
            >>> base_tensor = BaseTensor(data, orig_shape=(720, 1280))
            >>> gpu_tensor = base_tensor.cuda()
...
class Masks(BaseTensor):
    ...
    def __init__(self, masks, orig_shape) -> None:
        ...
        Examples:
            >>> import torch
            >>> from ultralytics_custom.engine.results import Masks
...
class Keypoints(BaseTensor):
    ...
    Examples:
        >>> import torch
        >>> from ultralytics_custom.engine.results import Keypoints
