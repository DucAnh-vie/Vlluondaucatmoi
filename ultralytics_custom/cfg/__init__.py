...
def get_cfg(cfg: Union[str, Path, Dict, SimpleNamespace] = DEFAULT_CFG_DICT, overrides: Dict = None) -> SimpleNamespace:
    ...
    Examples:
        >>> from ultralytics_custom.cfg import get_cfg
...
def handle_yolo_hub(args: List[str]) -> None:
    ...
    Notes:
        - The function imports the 'hub' module from ultralytics_custom to perform login and logout operations.
        - For the 'login' command, if no API key is provided, an empty string is passed to the login function.
        - The 'logout' command does not require any additional arguments.
    """
    from ultralytics_custom import hub
...
def handle_yolo_solutions(args: List[str]) -> None:
    ...
    if solution_name == "inference":
        ...
        from ultralytics_custom import solutions
...
def entrypoint(debug: str = "") -> None:
    ...
    if "rtdetr" in stem:  # guess architecture
        from ultralytics_custom import RTDETR

        model = RTDETR(model)  # no task argument
    elif "fastsam" in stem:
        from ultralytics_custom import FastSAM

        model = FastSAM(model)
    elif "sam_" in stem or "sam2_" in stem or "sam2.1_" in stem:
        from ultralytics_custom import SAM
