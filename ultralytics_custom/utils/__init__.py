...
from ultralytics_custom.utils.patches import imread, imshow, imwrite, torch_load, torch_save  # for patches
...
LOGGING_NAME = "ultralytics_custom"
...
HELP_MSG = """
    ...
    1. Install the ultralytics_custom package:

        pip install ultralytics_custom
...
class TQDM(rich.tqdm if TQDM_RICH else tqdm.tqdm):
    ...
    Examples:
        >>> from ultralytics_custom.utils import TQDM
...
class IterableSimpleNamespace(SimpleNamespace):
    ...
    def __getattr__(self, attr):
        ...
        raise AttributeError(
            f"""
            '{name}' object has no attribute '{attr}'. This may be caused by a modified or out of date ultralytics_custom
            'default.yaml' file.\nPlease update your code with 'pip install -U ultralytics_custom' and if necessary replace
...
def set_logging(name="LOGGING_NAME", verbose=True):
    ...
    Examples:
        >>> set_logging(name="ultralytics_custom", verbose=True)
        >>> logger = logging.getLogger("ultralytics_custom")
...
class ThreadingLocked:
    ...
    Examples:
        >>> from ultralytics_custom.utils import ThreadingLocked
        >>> @ThreadingLocked()
        >>> def my_function():
        ...    # Your code here
    """
...
class SettingsManager(JSONDict):
    ...
    def __init__(self, file=SETTINGS_FILE, version="0.0.6"):
        ...
        from ultralytics_custom.utils.torch_utils import torch_distributed_zero_first
    ...
    def _validate_settings(self):
        ...
        if not (correct_keys and correct_types and correct_version):
            LOGGER.warning(
                ...
                f"with your settings or a recent ultralytics_custom package update. {self.help_msg}"
            )
            self.reset()
...
def vscode_msg(ext="ultralytics_custom.ultralytics_custom-snippets") -> str:
