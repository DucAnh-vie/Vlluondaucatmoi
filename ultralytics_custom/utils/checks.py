...
def parse_requirements(file_path=ROOT.parent / "requirements.txt", package=""):
    ...
    Examples:
        >>> from ultralytics_custom.utils.checks import parse_requirements
        >>> parse_requirements(package="ultralytics_custom")
    """
    if package:
...
def check_version(
    ...
    if not current:  # if current is '' or None
        ...
        return True
    elif not current[0].isdigit():  # current is package name rather than version string, i.e. current='ultralytics_custom'
        try:
...
def check_latest_pypi_version(package_name="ultralytics_custom"):
    """
...
def check_pip_update_available():
    """
    Checks if a new version of the ultralytics_custom package is available on PyPI.
        ...
        try:
            ...
            if check_version(__version__, f"<{latest}"):  # check if current version is < latest version
                LOGGER.info(
                    ...
                    f"Update with 'pip install -U ultralytics_custom'"
                )
                return True
        except Exception:
            pass
...
def check_requirements(requirements=ROOT.parent / "requirements.txt", exclude=(), install=True, cmds=""):
    ...
    Examples:
        >>> from ultralytics_custom.utils.checks import check_requirements

        Check a requirements.txt file
        >>> check_requirements("path/to/requirements.txt")

        Check a single package
        >>> check_requirements("ultralytics_custom>=8.0.0")

        Check multiple packages
        >>> check_requirements(["numpy", "ultralytics_custom>=8.0.0"])
    """
...
def check_yolo(verbose=True, device=""):
    ...
    from ultralytics_custom.utils.torch_utils import select_device
...
def collect_system_info():
    ...
    from ultralytics_custom.utils import ENVIRONMENT  # scope to avoid circular import
    from ultralytics_custom.utils.torch_utils import get_cpu_info, get_gpu_info
    ...
    package_info = {}
    for r in parse_requirements(package="ultralytics_custom"):
        try:
...
def check_amp(model):
    ...
    Examples:
        ...
        >>> from ultralytics_custom.utils.checks import check_amp
        >>> model = YOLO("yolo11n.pt").model.cuda()
        >>> check_amp(model)
    """
    from ultralytics_custom.utils.torch_utils import autocast
