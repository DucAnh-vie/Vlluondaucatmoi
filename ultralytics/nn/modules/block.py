...
class C2f(nn.Module):
    """Modified C2f block without input chunking."""

    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__()
        self.c = int(c2 * e)
        self.cv1 = Conv(c1, self.c, 1, 1)
        self.cv2 = Conv((1 + n) * self.c, c2, 1, 1)
        self.m = nn.ModuleList(Bottleneck(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))

    def forward(self, x):
        y = [self.cv1(x)]
        y.extend(m(y[-1]) for m in self.m)  # chain sequentially or use y[0]
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):
        ...
        self.shortcut = shortcut and c1 == c_

    def forward(self, x):
        out = self.cv1(x)
        if self.shortcut:
            out = out + x  # apply shortcut after first conv
        return self.cv2(out)
